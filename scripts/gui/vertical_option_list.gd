...
func _initialize():
	...
	cursor_pos_index = _fill_cursor_pos_index(_options)
...
func draw(cursor: Cursor):
	$Options.text[cursor_pos_index[current_option]] = SYMBOLS[cursor]
...
func deactivate():
	if save_last_option:
		...
		_remove_current_cursor()
	$VerticalOptionInput.deactivate()
